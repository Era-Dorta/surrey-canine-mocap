...
inline void get_file_names(std::string path_in, std::vector<std::string>* file_names)
{
...
inline void get_dir_names(std::string path_in, std::vector<std::string>* file_names)
{
	boost::filesystem::path path(path_in.c_str());
	boost::filesystem::directory_iterator end_iter;
	if ( boost::filesystem::exists(path) && boost::filesystem::is_directory(path))
...
inline osg::Group* create_3D_label(std::string text, osg::Vec3 offset, osg::Vec3 colour)
{
	...
	osg::ref_ptr<osgText::Font3D> g_font =
				osgText::readFont3DFile("FreeSans.ttf");
...
inline void enable_splat_rendering(osg::ref_ptr<osg::Geode> geometry_in)
{
	...
	//Vertex shader:
	//-------------------------
	static const char* pass_through_vertex =
	{
		"#version 120\n"
...
//	//------------------
	...
	splat_prog->addShader(
			new osg::Shader(osg::Shader::VERTEX, pass_through_vertex) );
	splat_prog->addShader(
			new osg::Shader(osg::Shader::GEOMETRY, splat_geom) );
	splat_prog->addShader(
				new osg::Shader(osg::Shader::FRAGMENT,
						//colour_frag));
						normal_colour_frag));
	splat_prog->setParameter( GL_GEOMETRY_VERTICES_OUT_EXT, 2*num_segments+2 );
	...
	stateset2->addUniform(
			new osg::Uniform("num_segments", int(num_segments) ));
	stateset2->addUniform(
			new osg::Uniform("pi_2", float(3.14159*2)) );
	//------------------

}


inline void surfel_ply_write(osg::ref_ptr<osg::Geode> geom_in, std::string file_name)
{
	...
	out_file << "element vertex " << geom_in->getDrawable(0)->asGeometry()->getVertexArray()->getNumElements() << std::endl;
	...
	osg::Vec3Array* vertex_array = static_cast<osg::Vec3Array*>(geom_in->getDrawable(0)->asGeometry()->getVertexArray());
	osg::Vec3Array* normal_array = static_cast<osg::Vec3Array*>(geom_in->getDrawable(0)->asGeometry()->getNormalArray());
...
//	in_file << "end_header" << std::endl;
	...
	//------------------------
	osg::ref_ptr<osg::Vec3Array> vertex_array = new osg::Vec3Array(num_elements);
	osg::ref_ptr<osg::Vec3Array> normal_array = new osg::Vec3Array(num_elements);
	osg::ref_ptr<osg::Vec4Array> colour_array = new osg::Vec4Array(num_elements);
	osg::ref_ptr<osg::Vec3Array> attrib_array = new osg::Vec3Array(num_elements);
	//------------------------
