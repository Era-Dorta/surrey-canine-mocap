...
RenderPOV::RenderPOV(int tgt_width, int tgt_height)
{
	...
	depth_buffer_tgt_cam_pov->allocateImage( tgt_width, tgt_height, 1, GL_DEPTH_COMPONENT, GL_FLOAT );
...
void RenderPOV::render_pov(
		osg::ref_ptr<osg::Geode> geometry, cv::Mat* map_tgt,
		osg::Matrix3 K_tgt, osg::Matrix T_src2tgt)
{
	...
	convert_depth_buffer_to_mm(map_tgt->cols, map_tgt->rows, depth_buffer_tgt_cam_pov, map_tgt);
...
void RenderPOV::set_camera_as_pov(osg::Camera* camera, osg::Matrix3 K, int width, int height, float near, float far)
{
...
osg::Matrix RenderPOV::get_cam_proj_mat(
		osg::Matrix3 K, int width, int height, float near, float far)
{
	...
	const float pmat_val[16] =
	{alpha,		0, -x_0,	0,
			0,		 -beta, -y_0,    0,//NB - changed from 0, beta, -y_0, 0,
			0,			0,	  X,	Y,
			0,      	0,   -1,	0};
	...
	proj_mat = pmat_transposed_matrix*(osg::Matrix::ortho(0,width,height,0,near,far));//Why this order??

	return proj_mat;
}

void RenderPOV::convert_depth_buffer_to_mm(
		int width, int height, osg::Image* osg_float, cv::Mat* cv_16_bit)
{

	//Extract correctly scaled 16-bit depth image from depth buffer:
	//-------

	cv::Mat depth(height, width, CV_32F, osg_float->data(), width*sizeof(float));
