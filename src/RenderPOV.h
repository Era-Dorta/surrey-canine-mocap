...
private:
	void set_camera_as_pov(
			osg::Camera* camera, osg::Matrix3 K, int width, int height, float near, float far);
	osg::Matrix get_cam_proj_mat(
			osg::Matrix3 K, int width, int height, float near, float far);
	void convert_depth_buffer_to_mm(
