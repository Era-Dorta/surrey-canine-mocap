...
bool LegFitter::fit_leg_position_go_up_y(Skeleton::Skel_Leg leg, int paw_index,
	...
	bone_lengths[0] = skeleton->get_node(leg)->get_length();
	bone_lengths[1] = skeleton->get_node(leg - 1)->get_length();

	osg::Vec3 bone_pos[3];
	//Get naive bone positions
	int num_valid = bone_pos_finder.find_leg_lower_3_joints(cloud,
			leg_points_index, bone_lengths, bone_pos, bone_pos,
			BonePosFinder::Simple);
	...
	bone_pos_finder.find_leg_lower_3_joints(cloud, leg_points_index,
			bone_lengths, bone_pos, new_bone_pos, BonePosFinder::S_Line_Fit);
